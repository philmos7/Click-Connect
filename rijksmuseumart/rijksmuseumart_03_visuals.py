...
# Not yet   Compound Score Sentiment Analysis per Post Type
# Not yet   Sentiment Analysis and Topic Modelling
...
account_data       = f"{BASE_DIR}/rijksmuseumart_account.csv"
posts_data         = f"{BASE_DIR}/rijksmuseumart_posts.csv"
posts_latest       = f"{BASE_DIR}/rijksmuseumart_posts_latest_enriched.csv"
top10              = f"{BASE_DIR}/rijksmuseumart_top10.csv"
worst10            = f"{BASE_DIR}/rijksmuseumart_worst10.csv"
engagement_growth  = f"{BASE_DIR}/rijksmuseumart_engagement_growth.csv"
...
ax.set_title("Normalised Growth of Followers, Following & Posts", fontsize=13, fontweight="bold")
...
# Title
plt.title("Post Distribution by Post Type", fontsize=13, fontweight="bold")
...
fig.suptitle("Avg. Engagement of Posts by Day & Hour Posted (Excl. Reposts)", fontsize=16, fontweight='bold')
...
ax_bar.set_title(f"Average Normalised Engagement by Media Type and Engagement Type (Excl. Reposts)", fontsize=14, fontweight="bold", pad=12)
ax_bar.set_xlabel("Average Engagement per 1,000 Followers", fontsize=12, fontweight="bold")
...
ax.set_title(f"Average Engagement per 1,000 Followers by Post Type", fontsize=14, fontweight="bold", pad=12)
...
# Labels
ax.set_title("Weekly Engagement Growth Rates by Type", fontsize=14, fontweight="bold", pad=16)
...
if len(post_types_order) == 1:
    axes = [axes]  # Macht axes iterierbar bei nur einem Plot
...
fig.suptitle("Weekly Engagement Growth Rates by Engagement and Post Type", fontsize=14, fontweight="bold", y=.97)
...
fig.suptitle("Engagement Composition by Media Type and Post Type", fontsize=14, fontweight="bold", y=0.95)
...
fig, axes = plt.subplots(nrows=len(post_types), ncols=len(metrics), figsize=(18, 6 * len(post_types)), sharex=True)
if len(post_types) == 1:
    axes = np.array([axes])
...
# Main title
plt.suptitle("Post Length vs Engagement (per 1k Followers) by Post Type", fontsize=15, fontweight="bold")
plt.show()
...
n_plots = len(post_types)
fig = plt.figure(figsize=(9 * n_plots, 8))  # Width scales with number of post types
gs = GridSpec(2, n_plots, height_ratios=[6, 1], hspace=0.4, figure=fig)
...
fig.suptitle("Posting Frequency vs. Engagement per Post (Normalised) by Post Type", fontsize=15, fontweight="bold")
...
# )
# plt.title("Distribution of Reply Types", fontsize=13, fontweight="bold")
...
# )
# plt.title("Average Engagement per Post (per 1k Followers) by Reply Type", fontsize=13, fontweight="bold")
...
nltk_sw = set(stopwords.words('english', 'dutch'))
wc_sw = set(STOPWORDS)
...
for pt in post_types:
    if texts[pt].strip():
        plot_wordcloud(texts[pt], f"{pt} Posts Word Cloud", color_dict[pt])
    else:
...
plt.axis('off')
plt.title("Mixed Word Cloud by Post Type", fontsize=16, fontweight='bold')
