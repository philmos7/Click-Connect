...
account_data       = f"{BASE_DIR}/themerl_account.csv"
posts_data         = f"{BASE_DIR}/themerl_posts.csv"
posts_latest       = f"{BASE_DIR}/themerl_posts_latest_enriched.csv"
top10              = f"{BASE_DIR}/themerl_top10.csv"
worst10            = f"{BASE_DIR}/themerl_worst10.csv"
engagement_growth  = f"{BASE_DIR}/themerl_engagement_growth.csv"
...
ax.set_title("The MERL: Normalised Growth of Followers, Following & Posts", fontsize=13, fontweight="bold")
...
# Title
plt.title("The MERL: Post Distribution by Post Type", fontsize=13, fontweight="bold")
...
post_types = ['Original', 'Reply', 'Quote']
...
fig.suptitle("The MERL: Avg. Engagement of Posts by Day & Hour Posted", fontsize=16, fontweight='bold')
...
# Define engagement group colour mapping using your custom palette
...
ax_bar.set_title(f"The MERL: Average Normalised Engagement by Media Type and Engagement Type (Excl. Reposts)", fontsize=14, fontweight="bold", pad=12)
...
ax.set_ylabel("Mean Engagement / 1,000 Followers", fontsize=12, fontweight="bold")

ax.set_title(f"The MERL: Average Engagement per 1k Followers by Post Type", fontsize=14, fontweight="bold", pad=12)
...
# # Optional: hide every second label if cluttered
...
# Labels
ax.set_title("The MERL: Weekly Engagement Growth Rates by Type", fontsize=14, fontweight="bold", pad=16)
...
post_types_order = ["Original", "Quote", "Reply"]
...
for ax, post_type in zip(axes, post_types_order):
    ...
    ax.set_title(f"{post_type}", fontweight="bold")
    ax.xaxis.set_major_locator(mdates.WeekdayLocator(byweekday=mdates.MO))
    ax.xaxis.set_major_formatter(mdates.DateFormatter('%d %b'))
    ax.grid(True, which="major", axis="x", linestyle="--", alpha=0.5)
...
fig.suptitle("The MERL: Weekly Engagement Growth Rates by Engagement and Post Type", fontsize=14, fontweight="bold", y=.97)
...
for ax, post_type in zip(axes, post_types):
    ...
    ax.set_title(post_type, fontsize=12, fontweight="bold")
...
fig.suptitle("The MERL: Engagement Composition by Media Type and Post Type", fontsize=14, fontweight="bold", y=0.95)
...
header_row = 0
header_height = 0.35  # adjust as needed
...
df_posts = df_posts[df_posts["post_type"].isin(["Original", "Reply", "Quote"])]
...
# Main title
plt.suptitle("The MERL: Post Length vs Engagement (per 1k Followers) by Post Type", fontsize=15, fontweight="bold")
plt.show()
...
# Define post types
post_types = ["Original", "Quote", "Reply"]

# Create figure and GridSpec
fig = plt.figure(figsize=(18, 8))
gs = GridSpec(2, 3, height_ratios=[6, 1], hspace=0.4, figure=fig)
...
# Annotation on Quote plot
axes[1].text(
    0.5, 0.9, "Limited data points for Quote posts",
    transform=axes[1].transAxes,
    fontsize=10, color="red",
    ha="center", va="center", alpha=0.7
)

fig.suptitle("The MERL: Posting Frequency vs. Engagement per Post (Normalised) by Post Type", fontsize=15, fontweight="bold")
...
plt.title("The MERL: Distribution of Reply Types", fontsize=13, fontweight="bold")
...
plt.title("The MERL: Average Engagement per Post (per 1k Followers) by Reply Type", fontsize=13, fontweight="bold")
...
wc_sw = set(STOPWORDS)
custom_sw = {
    "us", "thank", "thanks", "also", "would", "could", "please", "let", "much",
    "museum", "already", "get", "see", "like", "know", "go", "make", "one",
    "new", "time", "day", "post", "today", "tomorrow"
}
all_stopwords = nltk_sw.union(wc_sw).union(custom_sw)

text_col = 'text'
post_types = ["Original", "Reply", "Quote", "Repost"]
self_content = ["Original", "Reply", "Quote"]

# Clean text function
def clean_text(text):
    if pd.isna(text):
        return ""
    # Remove placeholders and trailing punctuation
    text = re.sub(r'\(?(@mention|self_link|platform_link|external_link)[\)\.,;:]*', '', text, flags=re.IGNORECASE)
    text = re.sub(r"\b(\w+)'s\b", r"\1", text)  # remove possessives
...
for pt in post_types:
    text_data = df.loc[df['post_type'] == pt, text_col].dropna().astype(str)
    texts[pt] = clean_text(' '.join(text_data))
...
# Plot word clouds for each post type
for pt in post_types:
    if texts[pt].strip():
        plot_wordcloud(texts[pt], f"The MERL: {pt} Posts Word Cloud", color_dict[pt])
    else:
        print(f"No text available for post type: {pt}")

# Frequency dicts per type
def get_freq_dict(text):
    return WordCloud(stopwords=all_stopwords).process_text(text)

freq_dicts = {pt: get_freq_dict(texts[pt]) for pt in post_types}

# Combine frequencies for all types
combined_freq = {}
word_type = {}
...
# Colour function for combined clouds
...
# Plot mixed word cloud (all post types)
...
plt.axis('off')
plt.title("The MERL: Mixed Word Cloud by Post Type", fontsize=16, fontweight='bold')
...
# --- Word cloud for self-content only ---

# Filter frequency dicts
texts_self = {pt: texts[pt] for pt in self_content}
freq_self = {pt: get_freq_dict(texts_self[pt]) for pt in texts_self}

combined_self_freq = {}
word_type_self = {}
self_words = set(word for freq in freq_self.values() for word in freq.keys())

for word in self_words:
    freqs = {pt: freq_self[pt].get(word, 0) for pt in self_content}
    dominant_pt = max(freqs, key=freqs.get)
    combined_self_freq[word] = freqs[dominant_pt]
    word_type_self[word] = dominant_pt

wc_self = WordCloud(**wc_params, stopwords=all_stopwords).generate_from_frequencies(combined_self_freq)

plt.figure(figsize=(14, 6))
plt.imshow(wc_self.recolor(color_func=GroupColorFunc(word_type_self, color_dict)), interpolation='bilinear')
plt.axis('off')
plt.title("The MERL: Word Cloud for Self-Created Posts", fontsize=16, fontweight='bold')

legend_self = [Patch(color=color_dict[pt], label=pt) for pt in self_content]
plt.legend(handles=legend_self, title="Post Type", fontsize=12, loc='center left', bbox_to_anchor=(1, 0.5))
plt.tight_layout()
plt.show()