...
target_did = 'did:plc:denuvqodvvnzxtuitumle4vs'
...
# Load post data
posts_df = pd.read_csv("/path/to/data/themerl_posts.csv")
...
#Keep only the latest post per URI
latest_df = posts_df.groupby('uri', as_index=False).last()

#Save top and worst 10 by engagement (excluding reposts)
...
top10_df.to_csv("/path/to/data/themerl_top10.csv", index=False)

worst10_df = non_reposts_df.sort_values(by='engagement_score', ascending=True).head(10)
worst10_df.to_csv("/path/to/data/themerl_worst10.csv", index=False)

#Engagement growth for URIs with multiple entries
...
engagement_growth_df.to_csv("/path/to/data/themerl_engagement_growth.csv", index=False)

#Enrich latest posts with reply type info
...
latest_df.to_csv("/path/to/data/themerl_posts_latest_enriched.csv", index=False)
...
print("Saved files:\n - themerl_posts_latest_enriched.csv\n - themerl_top10.csv\n - themerl_worst10.csv\n - themerl_engagement_growth.csv")
